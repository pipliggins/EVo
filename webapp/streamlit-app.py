...
def amend_env(file, env_dict):
    ...
    for param, val in env_dict.items():
        # forces the scientific notation to be output correctly to be read as a float
        if type(val) is np.float64:
...
# # i.e. use 'disabled' to grey out options which are not
# # valid based on the run options.
...
if "default_sollaws" not in st.session_state:
    ...
    st.session_state.no_loaded_data = True

title_image, _, title_col = st.columns([4, 1, 15])

with title_image:
    st.image("webapp/evo_logo.png")
...
with col1:
    ...
    with st.expander("Major Oxide Composition", expanded=False):
    ...
    with st.expander("Starting conditions", expanded=True):
        t_start = st.number_input(
            "Temperature (K)", min_value=600.0, max_value=2000.0, value=1473.15
        )
        # work out how to grey this out if you don't need to select a starting pressure
        p_start = st.number_input(
            "Starting Pressure (bar)", min_value=1.0, max_value=10000.0, value=3000.0
        )
        p_stop = st.number_input(
            "Final Pressure (bar)", min_value=1e-5, max_value=10000.0, value=1.0
        )
        dp_max = st.number_input(
            "Maximum pressure step size (bar)",
            min_value=1.0,
            max_value=100.0,
            value=100.0,
        )
        dp_min = st.number_input(
            "Minimum pressure step size (bar)",
            min_value=1e-5,
            max_value=10.0,
            value=0.1,
        )
        ...
        wgt = st.number_input(
            "Gas weight fraction at starting pressure",
            min_value=0.000000001,
            max_value=1.0,
            value=0.00001,
        )
        mass = st.number_input(
            "Mass of the magma packet (g)", min_value=0.0, value=100.0
        )
...
with col2:
    ...
    run_type = st.selectbox(
        "Run Type - Closed System or Open System", ("closed", "open")
    )
    if run_type == "Open system":
        loss_frac = st.number_input(
            "Fraction of gas phase lost at each P step during open system degassing",
            min_value=0.0,
            max_value=0.9999,
            value=0.99,
        )
    ...
    env_options.update(
        {"COMPOSITION": rock_type, "RUN_TYPE": run_type, "SINGLE_STEP": single_step}
    )

    saturation = st.radio(
        "Choose how to find the starting pressure:",
        (
            "Set  manually",
            "Find volatile saturation pressure",
            "Set volatile element mass, and find volatile saturation pressure",
        ),
    )
    ...
    gas_sys = st.selectbox(
        "Select the volatile element system:", ("OH", "COH", "SOH", "COHS", "COHSN")
    )
    fe_equil = st.checkbox("Allow redox equilibration between gas and melt?")
    s_sat_warn = st.checkbox(
        "Stop the run if sulfide saturation is reached? "
        + "(EVo is not suitable for use under sulfide-saturated conditions)"
    )
    ...
    sol_laws = st.checkbox(
        "Use default solubility laws? (if unchecked, dropdown options will be offered)",
        value=True,
        key="default_sollaws",
    )
    # These will be greyed out if default solubility laws are chosen

    with st.expander("Solubility laws", expanded=not (sol_laws)):
        fo2_model = st.selectbox(
            "Ferric/Ferrous -> fO2 conversions model",
            ("kc1991", "righter2015"),
            disabled=st.session_state.default_sollaws,
        )
        h2o_model = st.selectbox(
            "H2O solubility law",
            ("burguisser2015",),
            disabled=st.session_state.default_sollaws,
        )
        h2_model = st.selectbox(
            "H2 solubility law",
            ("gaillard2003", "burguisser2015"),
            disabled=st.session_state.default_sollaws,
        )
        co2_model = st.selectbox(
            "CO2 solubility law",
            ("burguisser2015", "eguchi2018"),
            disabled=st.session_state.default_sollaws,
        )
        co_model = st.selectbox(
            "CO solubility law",
            ("armstrong2015", "None"),
            disabled=st.session_state.default_sollaws,
        )
        ch4_model = st.selectbox(
            "CH4 solubility law",
            ("ardia2013", "None"),
            disabled=st.session_state.default_sollaws,
        )
        sulfide_model = st.selectbox(
            "sulfide capacity law",
            ("oneill2020", "oneill2002"),
            disabled=st.session_state.default_sollaws,
        )
        n2_model = st.selectbox(
            "N2 solubility law",
            ("libourel2003",),
            disabled=st.session_state.default_sollaws,
        )
...
with col3:
    ...
    buffer_type = st.selectbox(
        "Select the mineral buffer for the oxygen fugacity:", ("FMQ", "IW", "NNO")
    )
    ...
    vol_setup = st.radio(
        "Choose a way to set the system volatile content:",
        ("Set gas phase", "Set melt content", "Set atomic weight fractions"),
    )
    ...
    if vol_setup == "Set gas phase":
        ...
        sulfur_start = st.number_input(
            "sulfur", value=0.0, label_visibility="collapsed"
        )

        nitrogen_set = st.checkbox("Set melt N2 content (wt fraction)", value=False)
        nitrogen_start = st.number_input(
            "nitrogen", value=0.0, label_visibility="collapsed"
        )

        graphite_saturated = st.checkbox("Is the melt graphite saturated?", value=False)
        graphite_start = st.number_input(
            "Weight fraction of graphite in the melt", value=0.0
        )
        ...
        st.subheader(
            "Set the mass fraction of each volatile element in the system (ppm)"
        )
...
_, col4, _, col5, _ = st.columns([1, 1, 1, 1, 1])

with col4:
    load_data = st.button("Load input data", use_container_width=True)
    if load_data:
        st.session_state.no_loaded_data = False
        filepath = filepath = Path(__file__).parent
        amend_chem(filepath / "chem.yaml", oxides)
        env_params = env_options | models | volatile_params
        amend_env(filepath / "env.yaml", env_params)  # update to get all options
...
with col5:
    ...
    run_evo = st.button(
        "Run EVo",
        type="primary",
        disabled=st.session_state.no_loaded_data,
    ...
    if run_evo:
        ...
        filepath = filepath = Path(__file__).parent
        df = evo.main(
            filepath / "chem.yaml", filepath / "env.yaml", filepath / "output.yaml"
        )

if run_evo:
    st.header("Results:")
    st.dataframe(df.style.format("{:.3e}"))
    st.download_button(
        "Download results", df.to_csv(index=False), "results.csv", "text/csv"
    )
    ...
    with col1:
        ...
        fig.update_layout(
            title="Gas composition vs pressure",
            xaxis_title="Gas composition (mol fraction)",
    ...
    with col2:
        ...
        fig.update_layout(
            title="Gas volume vs pressure",
            xaxis_title="Gas volume %",
            yaxis_title="Pressure (bar)",
        )
